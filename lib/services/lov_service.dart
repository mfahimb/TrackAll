...
class LovService {
  ...
  /// FIX: login_page.dart saves the user as 'userId'.
  /// Check that key first, then fall back to legacy keys for safety.
  Future<String?> _getLoginUser() async {
    final prefs = await SharedPreferences.getInstance();
    return prefs.getString("userId")          // ← primary (set by login_page)
        ?? prefs.getString("staff_id")        // ← legacy fallbacks
  ...
  Future<List<Map<String, String>>> fetchQcLov({
    ...
    try {
      ...
      // Resolve login user — always prefers the passed-in value,
      // then falls back to the shared prefs key written by login_page.
      Future<String?> resolveUser() async {
        if (appUserId != null && appUserId.trim().isNotEmpty) return appUserId;
      ...
      switch (upperType) {
        ...
        case "QC_TYPE":
          final lu = await resolveUser();
          if (lu != null && lu.isNotEmpty) queryParams["P_APP_USER"] = lu;
          break;
      ...
      debugPrint("🌐 QC LOV [$upperType] => $uri");
      final response = await http.get(uri);
      if (response.statusCode != 200) {
        debugPrint("❌ QC LOV [$upperType] HTTP ${response.statusCode}");
        return [];
      }

      final decoded = jsonDecode(response.body);
      if (!decoded.containsKey(upperType)) {
        debugPrint("❌ QC LOV: key '$upperType' missing. Keys: ${decoded.keys.toList()}");
        return [];
      }

      final rawList = decoded[upperType] as List;
      debugPrint("📦 QC LOV [$upperType]: ${rawList.length} items");
      if (rawList.isNotEmpty) {
        debugPrint("🔍 QC LOV [$upperType] first raw keys: ${(rawList.first as Map).keys.toList()}");
        debugPrint("🔍 QC LOV [$upperType] first raw item: ${rawList.first}");
      }

      return rawList.map<Map<String, String>>((raw) {
        final m = raw as Map;
        String id = "", label = "";
        ...
        } else if (upperType == "QC_TYPE") {
          id    = m["TP_ID"]?.toString()     ?? "";
          label = m["TYPE_NAME"]?.toString() ?? "";
        } else if (upperType == "QC_SIZE") {
          id    = m["OCSI_ID"]?.toString()   ?? m["OCSI_SIZE"]?.toString() ?? "";
          label = m["OCSI_SIZE"]?.toString() ?? "";
        } else if (upperType == "QC_ISSUE") {
  ...
  Future<List<Map<String, String>>> fetchProductionLov({
    ...
    try {
      ...
      // Helper: resolve user with correct key priority
      Future<String?> resolveUser() async {
        if (appUserId != null && appUserId.trim().isNotEmpty) return appUserId;
        return _getLoginUser();
      }
    ...
    try {
      ...
      switch (upperType) {
        ...
        case "COUNTRY_SIZE":
          ...
          if (planNo != null && planNo.isNotEmpty) queryParams["P_PLAN_NO"] = planNo;
          break;

        case "PLAN_COUNTRY_SIZE":
          if (processId == null || processId.isEmpty ||
              biiId      == null || biiId.isEmpty      ||
              countryId  == null || countryId.isEmpty  ||
              planNo     == null || planNo.isEmpty) {
            debugPrint("⚠️ PLAN_COUNTRY_SIZE: processId, biiId, countryId and planNo are all required");
            return [];
          }
          queryParams["p_process_id"] = processId;
          queryParams["p_bii_id"]     = biiId;
          queryParams["p_country_id"] = countryId;
          queryParams["P_PLAN_NO"]    = planNo;
          break;
  ...
  // ===============================================================
// ===============================================================
// FETCH REMAINING QTY (original — used by normal production entry)
// ===============================================================
Future<String> fetchRemainingQty({
  ...
  // ===============================================================
// FETCH REMAINING QTY FOR PLAN — used by plan no. wise production only
// ===============================================================
Future<String> fetchPlanRemainingQty({
  required String biiId,
  required String processId,
  required String sizeId,
  String? planNo,
  ...
  try {
    ...
    request.headers.addAll({
      ...
      'p_process_id':  processId,
      'p_size':        sizeId,
      'P_QRYTYP':      'REM_PLAN_QTY',
      if (planNo != null && planNo.isNotEmpty) 'P_PLAN_NO': planNo,
      'LOGIN_COMPANY': company,
    });
    ...
    final decoded = jsonDecode(response.body);
    if (!decoded.containsKey("REM_PLAN_QTY")) return "0";

    final rawList = decoded["REM_PLAN_QTY"] as List;
    if (rawList.isEmpty) return "0";

    final first = rawList.first as Map;
    for (final key in [
      "REAMING_QUANTITY",   // backend typo
      "REMAINING_QUANTITY",
      "REMAINING_QTY",
      "REM_QTY",
      "QTY",
    ...
    debugPrint("❌ PLAN REMAINING QTY EXCEPTION => $e");
    return "0";
  }
}
  ...
  Future<bool> saveQcEntry({
    ...
    try {
      ...
      if (selectedCompany == null || selectedCompany.isEmpty) {
        debugPrint("❌ QC SAVE BLOCKED: No company selected");
        return false;
      }
      if (imagePath == null && mobileImageFile == null && webImageBytes == null) {
        debugPrint("❌ QC SAVE BLOCKED: No image provided");
        return false;
      }
      ...
      if (imageBytes == null || imageBytes.isEmpty) {
        debugPrint("❌ QC SAVE BLOCKED: Failed to read image bytes");
        return false;
      }
      ...
      request.headers['pdqc_alter_qty']   = '0';
      request.headers['pdqc_order_id']    = orderNo;
      request.headers['pdqc_size']        = (sizeId != null && sizeId.isNotEmpty)
          ? sizeId
          : (size != null && size.isNotEmpty ? size : '0');
      request.headers['Content-Type']     = 'image/jpeg';
      request.bodyBytes = imageBytes;

      debugPrint("📡 QC SAVE REQUEST: $_uploadUrl");
      request.headers.forEach((k, v) {
        if (k != 'Content-Type') debugPrint("   $k: $v");
      });
      ...
      debugPrint("📡 QC SAVE RESPONSE: ${response.statusCode} | ${response.body}");
      ...
      if (response.statusCode == 200) {
        try {
          ...
          final success = decoded['success'] == true ||
              decoded['status']?.toString().toUpperCase() == 'SUCCESS' ||
              decoded['pdqc_id'] != null;
          debugPrint(success ? "✅ QC ENTRY SAVED" : "❌ QC SAVE FAILED: $decoded");
          return success;
        } catch (e) {
          debugPrint("⚠️ QC SAVE: non-JSON 200, treating as success");
          return true;
        }
      } else if (response.statusCode == 201) {
        debugPrint("✅ QC ENTRY SAVED (201)");
        return true;
      }

      debugPrint("❌ QC SAVE FAILED: HTTP ${response.statusCode}");
      return false;
    } on TimeoutException {
      debugPrint("❌ QC SAVE TIMEOUT");
      return false;
    } on http.ClientException catch (e) {
      debugPrint("❌ QC SAVE CLIENT EXCEPTION: $e");
      return false;
    } catch (e, st) {
      debugPrint("❌ QC SAVE EXCEPTION: $e\n$st");
      return false;
    }
  }
  ...
  String _extractId(Map m, String type) {
    for (final k in ["BII_ID", "R", "LINE_ID", "PROCESS_ID", "OCSI_SIZE", "SIZE_ID", "ID"]) {
  ...
  String _extractLabel(Map m, String type) {
    for (final k in [
      ...
      "OCSI_SIZE", "SIZE_NAME", "QC_TYPE_NAME", "TYPE_NAME",
      "ISSUE_TYPE_NAME", "ISSUE_NAME", "ITEM_DESC", "NAME",
    ]) {
      if (m.containsKey(k)) return m[k]?.toString() ?? "";
    }
    return "";
