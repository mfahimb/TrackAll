...
//
// API flow:
//   1. PACKING_PROCESS  — on init, no params
//   2. PACKING_BII      — after Process selected  (needs processId)
...
//   6. savePackingEntry — Save button (ONE POST, all sizes as array)
//
// Request sent to Packing_Production:
//   METHOD  : POST
//   HEADERS : P_COMPANY_ID, P_APP_USER, Content-Type: application/json
//   BODY    :
//   {
//     "pd_line_id":    215,
//     "pd_process_id": 179,
//     "pd_bii_id":     3105,
//     "pd_country":    6308,   ← R from PACKING_COUNTRY LOV
//     "sizes": [
//       { "size": "41", "qty": 10 },
//       { "size": "42", "qty": 12 }
//     ]
...
class _PackingProductionEntryPageState
  ...
  // ── R value from PACKING_COUNTRY LOV → pd_country in backend ───
  String? country;

  // ── Info fields from PACKING_BII response ───────────────────────
  ...
  Future<void> _generateSizes() async {
    ...
    try {
      ...
      // Sort sizes numerically then alphabetically
      data.sort((a, b) {
  ...
  // ================================================================
  // SAVE — single POST, all sizes in one JSON body
  // Credentials sent as HTTP headers (P_COMPANY_ID, P_APP_USER)
  // Body only contains: size + qty  (no remaining_qty)
  // ================================================================
  Future<void> _handleSave() async {
    ...
    debugPrint("📤 PACKING SAVE:");
    debugPrint("   lineId=$lineId  processId=$processId");
    ...
    // ── Only send size + qty — matches Postman body exactly ──────
    ...
    try {
      final ok = await _lovService.savePackingEntry(
        ...
        processId: processId!,
        biiId:     itemId!,
        country:   country ?? "0",  // R from PACKING_COUNTRY → pd_country
        appUser:   appUser ?? "",
        sizes:     sizesPayload,
      );
      ...
      if (ok) {
        ...
        // Reset qty inputs after successful save
  ...
  // ================================================================
  // COUNTRY DROPDOWN
  // id = R from PACKING_COUNTRY LOV → stored as both countryId & country
  // country value is sent as pd_country in the JSON body
  // ================================================================
                      ...
                      ...sizeRows.asMap().entries.map((e) {
                        ...
                        return Container(
                          ...
                          child: Row(children: [
                            ...
                            SizedBox(
                              width: colW[4],
                              child: Text(row.packingRemaining.toString(),
                                  textAlign: TextAlign.center,
                                  style: _cellStyle),
                            ),
                            SizedBox(
