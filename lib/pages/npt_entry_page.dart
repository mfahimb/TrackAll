...
// Shows a › arrow when text is cut off. Tap to scroll horizontally.
// =====================================================================
...
class _NptEntryPageState extends State<NptEntryPage> {
  ...
  Future<void> _loadInitialData() async {
    ...
    try {
      await Future.wait([
        _lovService.fetchLov(qryType: "BUILDING", appUserId: loginUserId)
            .then((v) => buildingList = v),
        _lovService.fetchLov(qryType: "PROCESS", appUserId: loginUserId)
            .then((v) => processList = v),
        _lovService.fetchLov(qryType: "DOWNTIME", appUserId: loginUserId)
            .then((v) => downtimeCauseList = v),
        _lovService.fetchLov(qryType: "RESP_DEPT", appUserId: loginUserId)
            .then((v) => responsibleDeptList = v),
        _lovService.fetchLov(qryType: "OPERATION", appUserId: loginUserId)
            .then((v) => operationCategoryList = v),
        _lovService.fetchLov(qryType: "RESP_USER", appUserId: loginUserId)
