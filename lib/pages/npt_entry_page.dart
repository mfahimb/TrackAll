...
class _NptEntryPageState extends State<NptEntryPage> {
  ...
  String? smv, remarks, numberOfOperators;
  ...
  bool _smvError = false;
  bool _opsError = false;
  ...
  Future<void> pickTime(bool isStart) async {
    ...
    if (picked != null) {
      setState(() {
        if (isStart) { startTime = picked; _startController.text = picked.format(context); }
        else         { endTime   = picked; _endController.text   = picked.format(context); }
      });
    }
  }
  ...
  void _clearAll() {
    setState(() {
      ...
      smv = null;  remarks = null;  numberOfOperators = null;
      startTime = null;           endTime = null;
      _smvError = false;          _opsError = false;
    });
  ...
  Future<void> _handleSave() async {
    final requiredFields = [
      buildingId, processId, categoryId, lineId, machineNo,
      deptId, responsibleUserId, causeLabel, smv,
      numberOfOperators, remarks, startTime, endTime,
    ];

    if (requiredFields.any((e) => e == null || (e is String && e.trim().isEmpty))) {
    ...
    if (int.tryParse(numberOfOperators!) == null || double.tryParse(smv!) == null) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
                ...
                child: Column(children: [
                  ...
                  Wrap(spacing: 12, runSpacing: 12, children: [
                    ...
                    _modernDropdown(context, "Category *", categoryLabel, operationCategoryList, (id, label) {
                      setState(() { categoryId = id; categoryLabel = label; });
                    }, width),
                    ...
                    // ── SMV — decimal ──────────────────────────────
                    _numberField("SMV *", _smvController, _smvError, (v) {
                      setState(() { smv = v; _smvError = false; });
                    }, width),
                    ...
                    // ── Operators — integers ───────────────────────
                    _numberField("Number of Operators *", _operatorsController, _opsError, (v) {
                      setState(() { numberOfOperators = v; _opsError = false; });
                    }, width, integersOnly: true),

                    _modernDropdown(context, "Responsible User *", responsibleUserLabel, responsibleUserList, (id, label) {
                      setState(() { responsibleUserId = id; responsibleUserLabel = label; });
                    }, width),

                    _textField("Remarks *", _remarksController, (v) => setState(() => remarks = v), width * 2 + 12),
  ...
  Widget _modernDropdown(BuildContext context, String label, String? value,
      List<Map<String, String>> list, void Function(String, String) onSelect, double width) {
    return SizedBox(
      width: width,
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Text(label,
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Text(label,
            ...
            style: TextStyle(
                ...
                color: hasError ? Colors.red.shade600 : Colors.black)),
        const SizedBox(height: 4),
        Container(
          height: 38,
          decoration: BoxDecoration(
        ...
        if (hasError)
          Padding(
            ...
            child: Text(
              integersOnly ? "Whole numbers only" : "Must be a number",
              style: TextStyle(fontSize: 9, fontWeight: FontWeight.w700, color: Colors.red.shade500),
            ),
          ),
      ]),
    );
