...
const List<LogCard> _allLogCards = [
  ...
  LogCard(
    ...
    columns: [
      ColDef("AEMP_NAME",     "Employee",  minWidth: 150),
      ColDef("LINE_NAME",     "Line",      minWidth: 130),
      ColDef("SHIFT_ID",      "Shift",     minWidth: 60),
      ColDef("CATEGORY_NAME", "Category",  minWidth: 120),
      ColDef("CID_NAME",      "Company",   minWidth: 140),
      ColDef("CREATED_AT",    "Time",      minWidth: 82, isDateTime: true),
    ],
  ),
];
...
class ActivityLogService {
  ...
  // ✅ FIX: Incentive needs dd-MM-yyyy date format + P_APP_USER
  if (qryType == "DAY_WISE_LINE_SETUP") {
    params = {
      "P_QRYTYP":      qryType,
  ...
  } else if (_noUserReports.contains(qryType)) {
    ...
    try {
      ...
      // ── Client-side date filter for Incentive (API ignores P_DATE) ──
      if (qryType == "DAY_WISE_LINE_SETUP" && result.isNotEmpty) {
        final target = DateFormat('yyyy-MM-dd').format(date);
        final filtered = result.where((row) {
          final icDate = row["IC_DATE"] ?? "";
          if (icDate.isEmpty) return false;
          return icDate.startsWith(target);
        }).toList();
        debugPrint("📅 [DAY_WISE_LINE_SETUP] ${filtered.length}/${result.length} rows for $target");
        return filtered;
      }
...
class _ActivityLogSectionState extends State<ActivityLogSection> {
  ...
  Widget build(BuildContext context) {
    ...
    Widget grid(List<LogCard> items) => GridView.count(
          crossAxisCount: 2,
          mainAxisSpacing: 8,
          crossAxisSpacing: 8,
          shrinkWrap: true,
    ...
    return Container(
      ...
      child: Row(
        children: cols.map((c) {
          ...
          } else if (widget.card.qryType == "DAY_WISE_LINE_SETUP" &&
              c.key == "SHIFT_ID") {
            // ── Shift badge for Incentive card — intrinsic size only ──
            final val = data![c.key] ?? "-";
            final isA = val.toUpperCase() == "A";
            cell = Align(
