...
class _QCEntryPageState extends State<QCEntryPage> {
  ...
  Future<void> _loadAppUser() async {
    ...
    // FIX: read appUser before loading any LOVs that depend on it
    appUser = prefs.getString('userId') ?? "";
    debugPrint("👤 appUser loaded: '$appUser'");
  ...
  Future<void> _loadItems() async {
    try {
      // FIX: pass appUserId so company-gated APIs work
  ...
  // FIX: pass appUserId so QC_TYPE API returns data
  Future<void> _loadQcTypes() async {
    try {
      ...
      debugPrint("✅ QC Types loaded: ${data.length} — $data");
  ...
  // FIX: pass appUserId for consistency; also clear+reload on qcType change
  ...
  try {
    final data = await _lovService.fetchQcLov(
      qryType: "QC_ISSUE",
      qcType: qcType,  // ✅ This should be "35" or "34"
      appUserId: appUser,
    );
    debugPrint("✅ Issue Types loaded: ${data.length} — $data");
  ...
  debugPrint("🔍 SAVING QC ENTRY:");
  debugPrint("   issueTypeId: $issueTypeId");
  debugPrint("   issueType: $issueType");
  debugPrint("   qcType: $qcType");
...
if (success) {
  ...
  _showError("❌ Server Error: The backend API has a bug (SQL syntax error at line 68). Please contact IT support to fix the 'qc_img_upload' endpoint.");
}
    } catch (e) {
      if (!mounted) return;
      setState(() => isLoading = false);
                  ...
                  return SingleChildScrollView(
                    ...
                    child: Wrap(
                      ...
                      children: [
                        ...
                        // FIX: Issue Type dropdown — only enabled after QC type is selected
                        _modernDropdownWithHint(
                          context,
                          "Issue Type *",
                          issueType,
                          issueTypeList,
                          issueTypeDisplayCount,
                          (id, label) {
                            setState(() { issueTypeId = id; issueType = label; });
                          },
                          width,
                          hint: qcType == null ? "Select QC Type first" : "Select",
                          enabled: qcType != null && qcType!.isNotEmpty,
                        ),
  ...
  Widget _modernDropdown(
    ...
    return _modernDropdownWithHint(
      context, label, value, list, displayCount, onSelect, width,
      hint: "Select", enabled: true,
    );
  }

  // FIX: new version with hint + enabled flag for Issue Type
  Widget _modernDropdownWithHint(
    BuildContext context, String label, String? value,
    List<Map<String, String>> list, int displayCount,
    void Function(String, String) onSelect, double width, {
    String hint = "Select",
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        InkWell(
          ...
          child: Container(
            height: 38,
            decoration: BoxDecoration(
              color: enabled ? Colors.white : const Color(0xFFE8EAED),
              borderRadius: BorderRadius.circular(8),
              border: Border.all(color: Colors.grey.shade300),
              boxShadow: enabled
                  ? const [BoxShadow(
            ...
            child: Row(children: [
              ...
              Padding(
                ...
                child: Icon(Icons.arrow_drop_down,
                    color: enabled ? Colors.black87 : Colors.grey, size: 20),
              ),
            ]),
          ),
        ),
        ...
        return Container(
          ...
          child: Column(children: [
            ...
            Expanded(
              child: filtered.isEmpty
                  ? Center(child: Text(
                      items.isEmpty ? "No data available" : "No items found",
