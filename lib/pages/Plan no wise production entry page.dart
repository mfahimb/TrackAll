...
class _PlanNoWiseProductionEntryPageState
 ...
 Future<void> _loadRemainingQty() async {
  ...
  setState(() => isLoading = true);
  try {
    final qty = await _lovService.fetchPlanRemainingQty( // ← new method
      biiId:     itemId!,
      processId: processId!,
      sizeId:    sizeId!,
      planNo:    planNo,
  ...
  final plan = planList.firstWhere((p) => p["id"] == id, orElse: () => {});
  setState(() {
  ...
  _loadRemainingQty(); // ← ADD THIS so qty refreshes when plan changes
}
