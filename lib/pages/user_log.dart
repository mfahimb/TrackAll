  ...
  Future<Map<String, List<_LogEntry>>> fetchAll({
    ...
    final futures = _entryTypes.map((type) async {
      try {
        ...
        final List raw = decoded[type.qryType] ?? [];

        final entries = raw
            .where((r) => r != null && r is Map)
            .map((r) => Map<String, dynamic>.from(r as Map))
            .where((row) {
              final createdBy = (row["CREATED_BY"] ?? "").toString().trim().toLowerCase();
              return createdBy == createdById.trim().toLowerCase();
            })
            .map((row) {
              DateTime? dt;
              ...
              if (ts.isNotEmpty) {
                try {
                  // Server stores local BD time but may append Z (incorrectly).
                  // Strip any trailing Z/timezone and parse as plain local time.
...
class _CardsView extends StatelessWidget {
  ...
  static void _openDetail(
    ...
    showModalBottomSheet(
      ...
      builder: (_) => _DetailSheet(type: type, entries: entries),
    );
  }
}
...
class _TypeCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    // For Kanban: count Ready vs Not Ready
        ...
        child: Column(
          children: [
            ...
            Padding(
              ...
              child: Row(
                children: [
                  // Kanban: show Ready / Not Ready counts
                  if (isKanban) ...[
                    _StatBox(
                      label: "Ready",
                      value: "$readyCount",
...
class _EntryRow extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    // For Kanban, use LSH_LINE_STAT for colored badge; skip it from chips
    final String kanbanStatus =
        isKanban ? (entry.raw["LSH_LINE_STAT"]?.toString() ?? "") : "";
    final bool isReady =
        kanbanStatus.toLowerCase() == "ready";

    final chips = type.chipKeys
        .where((k) => !(isKanban && k == "LSH_LINE_STAT")) // status shown as badge
