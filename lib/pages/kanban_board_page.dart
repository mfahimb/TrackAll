...
class _KanbanBoardPageState extends State<KanbanBoardPage> with TickerProviderStateMixin {
  ...
  void initState() {
    ...
    _updateClock();
    _loadInitialData();
    _refreshTimer = Timer.periodic(const Duration(seconds: 10), (_) => _refreshLinesFromBackend());
  ...
  void _updateClock() {
    if (mounted) {
      setState(() {
        _currentTime = DateFormat('dd MMM yyyy|hh:mm:ss a').format(DateTime.now());
      });
    }
  }
  ...
  Future<void> _loadInitialData() async {
    ...
    try {
      final units = await _lovService.fetchLov(qryType: "BUILDING", appUserId: userId);
      ...
      for (final e in result.where((e) => e['LINE_PRE_STAT']?.toString() == 'Y')) {
        ...
        String lineName = (e['LINE_NAME']?.toString() ?? 'Unknown').toUpperCase();

        // (Keeping your existing unit mapping logic exactly as is...)
        if (finalUnitId == null || finalUnitId.isEmpty || finalUnitId == "null") {
          ...
          if (match != null) {
            ...
            final matchedUnit = units.firstWhere(
              (u) {
                ...
                return label.replaceAll(' ', '').contains(unitCode.toUpperCase());
              },
              orElse: () => {},
            );
            if (matchedUnit.isNotEmpty) finalUnitId = matchedUnit['id']?.toString();
          }
        }
        ...
        loaded.add(SOSLine(
          ...
          lastNotReadyElapsed: _parseDowntime(e['DOWNTIME']?.toString()),
  ...
  Future<void> _refreshLinesFromBackend() async {
    ...
    try {
      ...
      for (var line in lines) {
        final match = result.firstWhere((e) => e['LINE_ID'].toString() == line.id, orElse: () => {});
        if (match.isNotEmpty) {
  ...
  // Marquee and Auto-scroll methods (Keeping your originals)
  void _startMarquee() {
    _marqueeTimer = Timer.periodic(const Duration(milliseconds: 30), (_) {
      ...
      _marqueeController.jumpTo(currentOffset >= maxExtent ? 0 : currentOffset + 1);
    });
  }

  void _startAutoScroll() {
    _autoScrollTimer = Timer.periodic(const Duration(milliseconds: 60), (_) {
      if (!_scrollController.hasClients || _isPaused || filteredLines.isEmpty) return;
      final max = _scrollController.position.maxScrollExtent;
      if (max <= 0) return;
      final current = _scrollController.offset;
      _scrollController.jumpTo(current >= max ? 0 : current + 1);
    });
  }
...
void _handleScrollEnd() {
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    Container(
                      ...
                      child: Text(
                        line.lastComment.isEmpty ? "No specific reason provided for this downtime." : line.lastComment,
                        style: TextStyle(
                          fontSize: 18, 
                      ...
                      children: [
                        Row(
                          children: [
                            ...
                            Text("By: ${line.staffId}", style: const TextStyle(color: Colors.black54, fontWeight: FontWeight.w600)),
                          ],
                        ),
                        Row(
                          children: [
                            ...
                            const SizedBox(width: 6),
                            Text("Duration: ${line.timeText}", style: const TextStyle(color: Colors.black54, fontWeight: FontWeight.w600)),
                          ],
                        ),
                      ],
                    ),
              ...
              Padding(
                ...
                child: SizedBox(
                  ...
                  child: ElevatedButton(
                    ...
                    child: const Text("DISMISS", style: TextStyle(fontWeight: FontWeight.bold, letterSpacing: 1.1)),
                  ),
                ),
              ),
            ],
          ...
          Container(
            ...
            child: const Row(
              children: [
                ...
                SizedBox(width: 5),
                Text("Filter", style: TextStyle(fontWeight: FontWeight.bold, fontSize: 10, color: Colors.white, letterSpacing: 0.5)),
              ],
            ),
          ),
                                    ...
                                    child: Container(
                                      ...
                                      child: Row(
                                        ...
                                        children: [
                                          ...
                                          GestureDetector(
                                            onTap: () => setState(() => selectedUnits.remove(unit)),
                                            child: Container(
              ...
              children: [
                ...
                Container(
                  ...
                  child: ListView.separated(
                    ...
                    itemBuilder: (ct, i) {
                      final isSelected = selectedUnits.any((u) => u['id'] == unitList[i]['id']);
                      return Material(
                        ...
                        child: InkWell(
                          onTap: () {
                            setState(() {
                              if (isSelected) {
                                selectedUnits.removeWhere((u) => u['id'] == unitList[i]['id']);
                              } else {
                          ...
                          child: Container(
                            ...
                            decoration: BoxDecoration(
                              ...
                              border: isSelected 
                                ? Border(left: BorderSide(color: Colors.blue.shade600, width: 3))
                                : null,
                            ),
                            child: Row(
                              children: [
  ...
  DateTime? _parseDbDate(String? val) => (val == null || val.isEmpty) ? null : DateTime.parse(val.replaceAll('Z', ''));
  ...
  @override
  void dispose() {
    _autoScrollTimer?.cancel(); _marqueeTimer?.cancel(); _refreshTimer?.cancel(); _clockTimer?.cancel();
    _pulseController.dispose(); _scrollController.dispose(); _marqueeController.dispose();
    super.dispose();
  }
  ...
  return Scaffold(
    ...
    body: Column(
      children: [
        ...
        // --- NEW SECTION: Status Tabs Row ---
        Container(
          width: double.infinity,
          color: Colors.white,
          padding: const EdgeInsets.fromLTRB(16, 0, 16, 10), // Padding below filter bar
          child: Row(
            children: [
              _buildStatusTab('All', const Color(0xFF64748B)), // Slate color
              const SizedBox(width: 8),
              _buildStatusTab('Ready', Colors.green.shade600),
              const SizedBox(width: 8),
              _buildStatusTab('Not Ready', Colors.red.shade600),
              const Spacer(),
                        ...
                        child: NotificationListener<ScrollNotification>(
                          ...
                          child: GridView.builder(
                            ...
                            itemBuilder: (_, i) => _SOSCard(
                              ...
                              onTap: filteredLines[i].status == "Ready"
                                  ? null
                                  : () => _showCommentPopup(filteredLines[i]),
                            ),
                          ),
                        ),
                      ),
...
class SOSLine {
  ...
  void updateStatusFromBackend(String val, DateTime? start, String sId, DateTime? sNow, String cmnt, Duration? lastDowntime) {
    status = val;
    staffId = sId;
    lastComment = cmnt;
    serverNow = sNow;
  ...
  void _calculate() {
    if (backendNotReadyStart != null) {
      elapsed = (serverNow ?? DateTime.now()).difference(backendNotReadyStart!);
  ...
  IconData get statusIcon => status == "Ready"
      ...
      : (elapsed.inMinutes >= 5 ? Icons.error_rounded : Icons.warning_rounded);
}
...
class _SOSCard extends StatelessWidget {
  ...
  const _SOSCard({required this.line, this.onTap, required this.pulseAnimation});
                ...
                Padding(
                  ...
                  child: Column(
                    ...
                    children: [
                      Row(
                        children: [
                          Icon(line.statusIcon, color: Colors.white, size: 14),
                          const Spacer(),
                          if (isNotReady) const Icon(Icons.timer, color: Colors.white, size: 12),
                        ],
                      ),
                      ...
                      Container(
                        ...
                        decoration: BoxDecoration(
                          color: isNotReady ? Colors.orange.withOpacity(0.8) : Colors.green.withOpacity(0.8),
                          borderRadius: BorderRadius.circular(6),
                        ),
                        child: Center(
                          child: Text(
                      ...
                      Row(
                        children: [
                          const Icon(Icons.person, color: Colors.white70, size: 10),
                          const SizedBox(width: 4),
                          Expanded(
                            child: Text(
                              line.staffId,
                              style: TextStyle(fontSize: h * 0.045, color: Colors.white70),
                              overflow: TextOverflow.ellipsis,
                            ),
                          ),
                        ],
