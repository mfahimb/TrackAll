...
class _CtlNptEntryPageState extends State<CtlNptEntryPage> {
  ...
  // ── Track which mandatory fields are missing (for red highlight) ──
  ...
  static const _kRespUser      = 'respUser';   // ← NOW MANDATORY
  ...
  void _clearAll() {
    ...
    _gmtLossController.clear();
    _remarksController.clear();
  }

  // ── Save ─────────────────────────────────────────────────────────
  Future<void> _handleSave() async {
    // Build the missing-field set so UI highlights them in red.
    ...
    if (responsibleUserId == null)                               missing.add(_kRespUser);   // ← MANDATORY
                    ...
                    Wrap(
                      ...
                      children: [
                        _modernDropdown(context, "Building *", buildingLabel,
                            buildingList, _kBuilding, (id, label) {
                          setState(() {
                        ...
                        _modernDropdown(context, "Process *", processLabel,
                            processList, _kProcess, (id, label) {
                          setState(() {
                        ...
                        _modernDropdown(context, "Category *", categoryLabel,
                            operationCategoryList, _kCategory, (id, label) {
                          setState(() {
                            categoryId = id; categoryLabel = label;
                            _missingFields.remove(_kCategory);
                          });
                        }, width),

                        _modernDropdown(context, "Line No *", lineLabel,
                            lineList, _kLine, (id, label) {
                          setState(() {
                            lineId = id; lineLabel = label;
                            _missingFields.remove(_kLine);
                          });
                        }, width),

                        _modernDropdown(context, "Machine No *", machineLabel,
                            machineList, _kMachine, (id, label) {
                          setState(() {
                            machineNo = id; machineLabel = label;
                            _missingFields.remove(_kMachine);
                          });
                        }, width),

                        _modernDropdown(context, "Responsible Dept *",
                            deptLabel, responsibleDeptList, _kDept,
                            (id, label) {
                          setState(() {
                            deptId = id; deptLabel = label;
                            _missingFields.remove(_kDept);
                          });
                        }, width),

                        _timeField("Start Time *", _startController,
                            () => pickTime(true), width, _kStartTime,
                            icon: Icons.access_time),

                        _timeField("End Time *", _endController,
                            () => pickTime(false), width, _kEndTime,
                            icon: Icons.access_time),

                        _readOnlyField(
                            "Total (Min)", totalTimeFormatted, width),

                        _textField("SMV *", _smvController, _kSmv,
                            (v) {
                          smv = v;
                        ...
                        _modernDropdown(context, "Cause *", causeLabel,
                            downtimeCauseList, _kCause, (id, label) {
                          setState(() {
                            causeLabel = label;
                            _missingFields.remove(_kCause);
                          });
                        }, width),

                        _textField("Number of Operators *",
                            _operatorsController, _kOperators, (v) {
                          numberOfOperators = v;
                        ...
                        _modernDropdown(
                          context,
                          "Responsible User *",   // ← asterisk in label
                          responsibleUserLabel,
                          responsibleUserList,
                          _kRespUser,             // ← tracked for red border
                          (id, label) {
                            setState(() {
                              responsibleUserId    = id;
                              responsibleUserLabel = label;
                              _missingFields.remove(_kRespUser);
                            });
                          },
                          width,
                        ),

                        _textField("GMT Loss Qty *", _gmtLossController,
                            _kGmtLoss, (v) {
                          gmtLossQty = v;
                          if (v.isNotEmpty) setState(() => _missingFields.remove(_kGmtLoss));
                        }, width, isNumber: true),

                        _textField("Remarks *", _remarksController, _kRemarks,
                            (v) {
                          remarks = v;
  ...
  /// Returns red border color when [fieldKey] is in [_missingFields],
  /// otherwise the normal grey border.
  Border _fieldBorder(String fieldKey) {
    return _missingFields.contains(fieldKey)
  ...
  Widget _modernDropdown(
    ...
    return SizedBox(
      ...
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          // Label row — shows red asterisk hint when missing
          Row(
            children: [
              Text(
                label,
            ...
            child: Row(children: [
              Expanded(
                child: CtlOverflowScrollText(
                  ...
                  style: TextStyle(
                    fontSize: 12,
                    fontWeight: FontWeight.w600,
                    color: isMissing && controller.text.isEmpty
                        ? Colors.red.shade300
                        : Colors.black87,
                  ),
                ),
              ...
              if (icon != null)
                Padding(
                  ...
                  child: Icon(icon,
                      color: isMissing ? Colors.red.shade400 : Colors.black54,
                      size: 18),
                ),
            ]),
          ),
